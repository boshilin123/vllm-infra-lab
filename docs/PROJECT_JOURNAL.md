...
截至 Phase 4 确定性 8/8 对照结果整理前，远端最新提交为：

```text
9d0f499 bench: prepare deterministic dual-replica routing test
```

Phase 2、Phase 3 与 Phase 4 普通 Service 静态双副本结果均已推送。确定性双目标工具和事前假设已推送；本地正在整理 8/8 直连三轮正式结果，等待项目本人检查提交。
...
以上条目均已提交到 `origin/main`。确定性 8/8 正式结果与本轮文档更新当前尚未提交。
...
| Phase 4 多副本与弹性 | 进行中（静态与确定性路由对照已完成） | 安全审计、155 秒冷启动、普通 Service 容量/偏斜实验、确定性 8/8 因果对照、两次安全回退 | namespace 内最小路由/弹性策略或离线回放 |

Phase 2、Phase 3 已闭环。Phase 4 已分别证明“两卡容量有效”和“普通 Service 的瞬时连接分配偏斜导致尾延迟失败”；确定性 8/8 在近似吞吐下将 P95 TTFT/E2E 恢复到单副本 c8 水平。下一步先在离线设计中区分请求级路由与副本伸缩，再决定是否实现 namespace 范围内的最小策略；不安装公司集群级 Adapter/KEDA，也不增加第三张 GPU。
...
第十四步为离线方案选择：根据已经验证的 155 秒冷启动和连接级分流问题，把“何时扩到两个副本”与“请求到达后选哪个副本”拆成两个控制环。先确定最小实现及验收标准，再决定是否需要一次受控两卡验证；在此之前不修改集群。
