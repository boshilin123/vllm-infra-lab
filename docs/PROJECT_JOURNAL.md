...
截至本轮并发 8 整理前，远端最新提交为：

```text
905e970 data: add concurrency-4 benchmark and project journal
```

本地还有尚未推送的实验前文档提交 `13b0343`；并发 8 正式结果和本轮文档更新尚未提交。下一次 Git 检查点应由项目本人检查后统一提交并推送。
...
| 指标 | 并发 1 | 并发 2 | 并发 4 | 并发 8 |
| --- | ---: | ---: | ---: | ---: |
| 请求吞吐（req/s） | 0.221521 | 0.398863 | 0.769253 | 1.396846 |
| 输出吞吐（tok/s） | 28.354627 | 51.054496 | 98.464412 | 178.796299 |
| 总吞吐（tok/s） | 85.063881 | 153.163487 | 295.393237 | 536.388898 |
| Mean TTFT（ms） | 73.899712 | 134.906244 | 238.705613 | 452.407425 |
| Mean TPOT（ms） | 34.963472 | 38.413134 | 39.078182 | 39.898263 |
| Mean E2E（ms） | 4513.756706 | 5013.367304 | 5198.595218 | 5516.847473 |
| P95 E2E（ms） | 4528.058606 | 5025.727483 | 5215.563536 | 5554.009860 |
| 正式请求成功数 | 300/300 | 300/300 | 300/300 | 300/300 |

并发 1、2、4 已提交到远端；并发 8 原始结果和报告已生成，当前等待项目本人检查和提交。
...
在看到 c8 结果后制定的 Short 场景 SLO v1 不能用于把 c8 包装成预注册验证。该 SLO 从 c16 起生效：请求成功率 ≥99%、P95 TTFT ≤600 ms、P95 TPOT ≤50 ms、P95 E2E ≤6 s；吞吐 CV ≤3%是重复性门槛，不属于服务体验 SLO。按新阈值回看 c8 全部通过，其中 P95 TTFT 约 525 ms，距离边界最近。
...
9. 用户完成 c8 事前回答；Agent 修正文档行尾空格并把校正后的假设记录为本地提交 `13b0343`，该提交尚未推送。
10. c8 首次由 Agent 启动时未激活虚拟环境 PATH，runner 误用系统 vLLM，参数校验失败且没有发出请求；清理仅含 metadata 的无效目录后，使用 vLLM 0.9.1 客户端重新启动。
11. 完成并发 8 三轮实验，300/300 请求成功；Agent 监督 running/waiting、KV Cache 和 GPU 状态并生成汇总，当前等待用户检查和提交。
...
| `13b0343` | 记录并发 8 事前假设 | 修复文档格式并在实验前冻结判断；当前仅在本地 |

除明确标记为本地的 `13b0343` 外，其余条目已提交到 `origin/main`。并发 8 正式结果和本轮文档更新尚未提交。
...
| Phase 2 基准与参数实验 | 进行中 | 工具链、聚合、并发 1/2/4/8 | 提交 c8，继续 c16、长短输入和参数实验 |
...
git status --short
git diff --check
git diff -- README.md docs/EXPERIMENTS.md docs/PROJECT_JOURNAL.md
sed -n '1,300p' results/2026-09-05/20260905-174433-short-c8/README.md
```

注意：未跟踪的 c8 文件不会完整出现在普通 `git diff` 中，所以要单独查看报告，并用 `git status --short` 确认目录。

第二步，在确认内容准确后，由项目本人提交并推送 c8 结果和本轮文档。提交前使用 `git add` 后再次检查 staged diff；同时会把尚未推送的本地提交 `13b0343` 一并推到远端。

第三步，由项目本人回答第 11.3 节的 c16 事前问题。Agent 校正并写入假设后，再由项目本人新建 `vllm-c16` tmux 会话执行并发 16。启动前必须重新确认：
...
并发 16 结束后先按 SLO v1 和容量指标复盘，再决定进入长短输入或参数实验。不要跳过预测和解释直接连续运行后续档位。
...
- 构建固定 Token 长度、预热、三轮重复、种子隔离和 JSON/CSV 汇总的可复现压测流程；在单张 A10、256/128 Token 场景下，并发 1→8 的输出吞吐从 28.35 提高到 178.80 tok/s，同时 Mean E2E 从 4.51 s 增至 5.52 s，各档均为 300/300 请求成功。

第三条包含尚未提交的 c8 数据，在 c8 文件检查并提交后再用于正式简历。
