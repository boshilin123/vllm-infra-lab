...
第十三步正在准备确定性 8/8 对照：保持两个 A10 副本、每副本 mns8、总 c16、256/128 Token、每轮 100 个正式请求和三轮不变，只将入口从 ClusterIP Service 改为两条同时直连 Pod IP 的 c8 流。每个 target 每轮处理 50 个正式请求并使用相同 seed，确保工作量对称；逐 target 原始 JSON 保留，合并结果从 100 个逐请求样本重新计算 P95，吞吐按两个近同步 target 中较长的测量窗口计算。该步骤先完成离线工具校验，尚未重新扩容或生成实验结论。
