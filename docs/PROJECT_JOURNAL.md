...
本地 loopback mock backend 的 8 个专项测试全部通过：非流式完整生命周期、SSE 收到响应头后仍持有 lease、客户端强制断开清理、上游连接失败清理、健康摘除/恢复、全部不可用 503、失败请求不盲目重试，以及 16 个同时持有的 HTTP 请求严格形成 8/8。连同核心 7 个和弹性回放 4 个测试，当时全仓共 19 个测试通过。
...
用户同意以确定性直连 `334.824 tok/s`的 90%作为真实代理吞吐下限，即 `301.342 tok/s`；Short P95 TTFT/TPOT/E2E 继续使用已经冻结的 600/50/6000 ms，不因代理增加一跳而放宽。若 TTFT 例如降至 700 ms，只能说相对普通 Service 改善，不能说全部通过 SLO；多测几轮解决重复性，不能改变阈值判定。

仓库新增 `benchmark/scenarios/phase4-least-inflight.yaml`，固定 256/128 Token、c16、每轮 16 个预热、100 个正式请求、三轮重复和独立 `seed_offset=700000`。使用项目 vLLM 虚拟环境完成 dry-run，六条 warmup/measured 命令与种子均正确生成，没有发请求或创建结果目录。

为加强真实实验的路由证据，代理增加只读 `/_router/status`，返回每个后端的 `healthy`、`in_flight`和`selections_total`；状态查询不 acquire lease，也不访问后端。新增专项测试通过后，HTTP 适配共 9 个、路由核心 7 个、弹性回放 4 个，全仓共 20 个测试。

`docs/PHASE4_ROUTER_EXPERIMENT.md`冻结了实验问题、控制变量、验收线、证据要求、安全停止条件、tmux 四窗口设计和回退优先级。真实命令只能在动态审计后根据当时 Pod 名、Pod IP、GPU编号与 UUID完整生成；不向用户提供带未解析占位符的正式命令。本轮仍未访问 Kubernetes或占用第二张 GPU。

### 11.20 least-inflight 真实双副本结果与安全回退

2026-09-08 重新执行动态安全审计：节点 GPU capacity/allocatable 均为4，Kubernetes 当前整卡申请为2；容器内外 UUID 确认公司 `magic-pdf`位于物理GPU0、原项目副本位于GPU1，GPU2/3同时满足宿主机无计算进程和Kubernetes未分配。节点实际可用内存约82 GiB；虽然 memory limit 已有超配风险，但短时第二副本的 request 与实际余量可承受，因此只允许受监控实验，不据此声称长期容量保证。

用户本人通过 `deploy/phase4-static`执行扩容；`maxSurge=0`确保最多两个 vLLM Pod。新副本在约105秒模型加载窗口内出现22次 startup probe `connection refused`，随后 Ready 且重启为0；探针周期5秒、失败阈值120，因此这是端口监听前的预期启动状态，不是22次服务崩溃。新副本实际落在物理GPU2，GPU0公司服务状态和重启基线未变化。

真实代理先用两个并发 SSE 请求做 smoke：执行中 `in_flight=1/1`、两个后端 `running=1/1、waiting=0/0`，两请求均HTTP 200并收到 `[DONE]`，完成后 `in_flight=0/0`、选择计数1/1。随后执行预注册正式场景，结果目录为 `results/2026-09-08/20260908-002836-phase4-least-inflight-c16-mns8-r2/`。

三轮正式请求均为100/100成功；输出吞吐中位数335.349 tok/s，P95 TTFT/TPOT/E2E中位数456.041/42.438/5527.425 ms，吞吐CV为0.209%，全部通过301.342 tok/s和600/50/6000 ms冻结门槛。相对确定性8/8的334.824 tok/s保留约100.16%，应视为测量波动范围内持平，不能包装为代理提高吞吐；相对普通Service，P95 TTFT从5469.014 ms降至456.041 ms，P95 E2E从10741.813 ms降至5527.425 ms。

正式窗口51个五秒采样点显示两侧最大 `running=8/8`、`waiting=0/0`；代理正式窗口前后累计选择数从1/1变为173/184，增量分布约48.5%/51.5%，最终 `in_flight=0/0`。选择增量包含warmup、measured和vLLM benchmark自带initial test等代理请求，不能直接等同于300个正式样本。

实验结束后用户本人恢复base：Deployment回到1/1和`Recreate`，client删除，原项目Pod保持Ready且0重启，GPU2显存回到1 MiB；公司GPU0全程正式窗口51个样本均为5901 MiB、最大利用率0，对应公司Pod保持Ready且重启基线仍为1。至此真实入口的性能、运行时分流和安全回退三类门槛闭环。
...
| Phase 4 多副本与弹性 | 已完成当前安全范围 | 安全审计、普通Service与确定性8/8对照、离线1→2→1回放、least-inflight HTTP/SSE真实双副本实验、三次安全回退与20个测试 | 自动控制器因冷启动和共享环境边界不执行；生产化能力属于未来扩展 |

Phase 2、Phase 3 已闭环。Phase 4 在当前安全范围内也已闭环：证明两卡容量、普通Service瞬时分流瓶颈、least-inflight真实入口收益和155秒冷启动对短突发的限制，并在每次实验后恢复单副本。不安装公司集群级 Adapter/KEDA，不再为形式上的自动扩缩容占用公司GPU。
...
第十八步已完成离线设计：新增只读路由状态端点、独立真实入口场景和预注册实验文档；项目虚拟环境 dry-run成功，全仓 20 个测试与静态检查全部通过。下一步先提交该检查点，再执行只读安全审计。

第十九步已完成：重新审计共享资源后，新副本安全落在GPU2；真实least-inflight代理通过双请求SSE smoke和三轮正式c16实验。300/300正式请求成功，中位输出吞吐335.349 tok/s，P95 TTFT/TPOT/E2E为456.041/42.438/5527.425 ms；正式窗口51个五秒采样点两侧waiting均为0，选择占比约48.5%/51.5%，最终in-flight回到0/0。用户随后恢复base单副本、删除client并确认GPU2释放，公司GPU0与Pod状态未变化。
...
- 设计并实现least-inflight HTTP/SSE实验代理，真实接入双A10上的两个vLLM Pod；三轮300/300成功，以335.349 tok/s保留约100.16%的确定性直连吞吐，P95 TTFT/TPOT/E2E为456.041/42.438/5527.425 ms，51个五秒采样点两侧waiting均为0，并完成单副本安全回退。

并发扫描和 mns16 参数实验的原始数据与报告均已提交，可作为已固化的远端证据；面试前仍应从原始 JSON 独立复算一次。

### 13.2 目前不能声称

- 不能声称已经完成 HPA/KEDA、自定义指标自动弹性或生产级队列感知路由；真实least-inflight只验证了固定成本合成负载。
