...
| Phase 4 多副本与弹性 | 进行中（路由核心已离线验证） | 安全审计、双副本对照、两次安全回退、1→2→1 回放、线程安全 least-inflight/lease 核心与 11 个测试 | HTTP/流式适配和健康探测；是否执行真实入口或自动弹性实验待安全评估 |

Phase 2、Phase 3 已闭环。Phase 4 已证明两卡容量、入口分流因果和短突发冷启动限制，并完成 least-inflight 选择核心。下一步先决定是否实现最小 HTTP/流式适配；即使实现，也先用本地 mock backend，不安装公司集群级 Adapter/KEDA，不自动占用第二张 GPU。
...
第十六步先设计最小 HTTP 适配的验收边界：本地 mock 后端、非流式与流式生命周期、503、客户端断开和健康恢复。只有离线验收完成且项目本人再次确认共享资源安全，才讨论是否运行真实双副本入口实验。
