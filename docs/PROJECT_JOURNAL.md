...
截至并发 16 事前假设整理前，远端最新提交为：

```text
000f8c3 data: add concurrency-8 benchmark baseline
```

并发 8 正式结果、SLO v1 和相关文档均已推送；当前仅有并发 16 事前假设尚待项目本人检查和提交。
...
以上条目均已提交到 `origin/main`。并发 16 事前假设是当前待提交内容。
...
| Phase 2 基准与参数实验 | 进行中 | 工具链、聚合、并发 1/2/4/8 | 提交 c16 事前假设，执行 c16、长短输入和参数实验 |
...
git status --short
git diff --check
git diff -- docs/PROJECT_JOURNAL.md \
  results/2026-09-05/20260905-174433-short-c8/README.md
```

重点确认原始回答被如实保留，并理解“调度器 waiting 不等于 Prefill block 等待”“排队主要影响 TTFT/E2E 而非 TPOT”这两个校正。

第二步，在确认内容准确后，由项目本人提交并推送事前假设。提交前使用 `git add` 后再次检查 staged diff；这个 commit 将成为 c16 metadata 中记录的实验前代码状态。

第三步，由项目本人新建 `vllm-c16` tmux 会话执行并发 16。启动前必须重新确认：
...
第三条的 c8 原始数据和报告已提交，但仍应在面试前从原始 JSON 独立复算一次。
