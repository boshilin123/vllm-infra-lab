...
截至 Phase 4 路由与弹性控制环设计前，远端最新提交为：

```text
394d3f7 data: add deterministic dual-replica routing benchmark
```

Phase 2、Phase 3，以及 Phase 4 普通 Service 与确定性 8/8 双副本结果均已推送。当前开始离线设计请求级路由与副本伸缩两个控制环，尚未再次扩容或修改集群。
...
| Phase 4 多副本与弹性 | 进行中（离线策略回放已完成） | 安全审计、155 秒冷启动、普通 Service 容量/偏斜实验、确定性 8/8 因果对照、两次安全回退、1→2→1 状态机回放 | namespace 内最小 least-inflight 路由实现；是否执行真实自动弹性实验待安全评估 |

Phase 2、Phase 3 已闭环。Phase 4 已分别证明“两卡容量有效”和“普通 Service 的瞬时连接分配偏斜导致尾延迟失败”；确定性 8/8 在近似吞吐下将 P95 TTFT/E2E 恢复到单副本 c8 水平。离线回放又证明当前短突发结束早于反应式第二副本 Ready。下一步只在仓库内实现和测试最小 least-inflight 路由，不安装公司集群级 Adapter/KEDA，也不增加第三张 GPU。
...
第十五步是在仓库内实现 CPU-only 的最小 least-inflight 路由核心和故障路径测试，先用 mock backend 验证并发计数、轮询平局、流式关闭与后端摘除。完成离线验收后，再由项目本人决定是否值得短时占用第二张 GPU做一次真实入口验证。
