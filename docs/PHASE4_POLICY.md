...
只读 `/_router/status`返回每个后端的健康状态、当前 in-flight 和累计选择数，查询本身不进入路由选择。真实入口实验将用它补充逐请求路由证据，但该端点没有认证或 Prometheus 格式，不是生产管理接口。真实实验的冻结验收线与安全门见 `PHASE4_ROUTER_EXPERIMENT.md`。

该适配已通过临时 client Pod 接入两个真实 vLLM Pod 完成一次固定负载实验，但没有作为 Kubernetes 常驻入口部署，也没有 EndpointSlice 服务发现、生产级连接池/背压/限流、认证、指标、优雅摘流或 Token-aware 权重，因此不能称为已部署的生产队列感知负载均衡器。客户端断开是在下一次下游写入时被感知；上游长时间不产生 chunk 时，取消传播会相应延后。
