...
4. 在共享 GPU 集群完成资源审计后，以 `maxSurge=0`短时扩至两张 A10；普通 Service 将 c16 吞吐提高 85.69%至 332.91 tok/s，但瞬时 waiting 达 6/4。进一步设计确定性 8/8 对照，在吞吐基本不变时将 P95 TTFT 从 5.47 s降至 0.525 s、P95 E2E 从 10.74 s降至 5.54 s，验证请求级分流是横向容量转化为 SLO 收益的关键，并在实验后恢复单副本。
...
实现线程安全的 least-inflight 选择与请求生命周期 lease 核心，并增加标准库最小 HTTP/SSE 适配；本地 mock 测试验证 16 个同时持有的 HTTP 请求形成 8/8，以及非流式/SSE 完整生命周期、客户端断开、异常释放、健康摘除/恢复、503 和不盲目重试。当前结论严格限定为本地 mock 验证，不声称已经部署生产 HTTP 代理。
...
- 尚未部署生产队列感知负载均衡、HPA/KEDA 或自定义指标自动扩缩容；当前 HTTP/SSE 适配仅通过本地 mock 验证。
