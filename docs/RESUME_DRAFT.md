...
实现线程安全的 least-inflight 选择与请求生命周期 lease 核心，16 个同时持有的等成本请求稳定形成 8/8；用单元测试覆盖平局轮询、并发原子性、异常释放、健康摘除和全后端不可用。当前结论严格限定为路由核心，不声称已经部署生产 HTTP 代理。
...
- 尚未完成队列感知负载均衡、HPA/KEDA 或自定义指标自动扩缩容。
