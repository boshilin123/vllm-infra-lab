...
以下版本只引用已经完成并可复验的 Phase 1～4 静态实验事实：
...
4. 在共享 GPU 集群完成双侧资源审计后，以 `maxSurge=0`短时扩至两张 A10；c16 输出吞吐相对单副本提升 85.69%至 332.91 tok/s、300/300 请求成功。结合每副本 5 秒指标发现整场请求虽约 50/50，瞬时 waiting 仍达 6/4并导致 TTFT/E2E SLO失败，定位连接级分流而非计算容量为新瓶颈，随后安全恢复单副本。
