...
本目录存放结果校验、统计与绘图逻辑。分析脚本只读取脱敏后的实验元数据、客户端结果和 Prometheus 导出数据，不直接依赖集群凭据。
...
- `phase4-routing-comparison.svg`：同一双 A10 固定负载下，普通 Service、确定性 8/8 直连与真实 least-inflight 代理对照。
...
![Phase 4 双副本路由对照](generated/phase4-routing-comparison.svg)

Phase 4 图只支持当前固定随机 Token 负载下的结论，不外推多轮对话、混合长度或生产流量。
